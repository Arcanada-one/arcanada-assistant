 ...
 * for `${VAR:-}`, so without this a blank `MUNERAL_PROJECT_ID` — the normal
 * "no narrowing" case — would fail uuid validation and take the whole app down
 * at boot.
 */
...
const envSchema = z.object({
   ...
   * mode.
   */
  MUNERAL_AGENT_KEY_FILE: optionalEnv(z.string().min(1)),
  /**
   ...
   */
  MUNERA_API_TOKEN: optionalEnv(z.string().min(1)),
  /** Narrows the proactive reads to one Muneral project. Unset ⇒ no narrowing. */
  MUNERAL_PROJECT_ID: optionalEnv(z.string().uuid()),
