...
- Postgres `assistant_app` role: `NOSUPERUSER NOCREATEROLE NOCREATEDB` (ARCA-0039), enforced via `ops/postgres-init/01-restrict-role.sql` — `dev-tools/check-postgres-role-restriction.sh` is a static gate. **Caveat:** `docker-entrypoint-initdb.d` scripts only run on first boot against an empty `postgres-data` volume. An already-initialized PROD role stays SUPERUSER until an operator runs the same `ALTER ROLE` by hand:
  ```bash
  docker compose exec postgres psql -U assistant_app -d assistant -c \
